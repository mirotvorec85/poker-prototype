...
public class BBAllInController : MonoBehaviour {
 ...
 [System.Serializable]
  public class PlayersCValue {
  	...
  	public List<PlayersCValue> playersCVal = new List<BBAllInController.PlayersCValue>();
...
#region allInFunctions

	public bool checkIfCanAllInForAIPlayers(int playerID,float currentPlayerTotalMoney, float allInInActionValue) {

		int toCall = UnityEngine.Random.Range(1,3);

		if(currentPlayerTotalMoney >= allInInActionValue) {
				Debug.Log("--- 2 --- executeAIOnFinishTurnAction xxxxxxxxxxxxxxxxxxxxxx allInInAction xxxxxxxxxxxxxxxxxxxxxx player : " + playerID +
				" playerMoney : " + currentPlayerTotalMoney +
				" allIn Value : " + allInInActionValue +
				" toCall : " + toCall
				);
                     if(toCall == 2) {
                       StartCoroutine( executeAllInCallForAIPlayer(playerID) );
				       return true;
                     } else {
					   return false;
                     }
	...
	public IEnumerator executeAllInCallForAIPlayer(int playerID) {

		BBGC.playerDataList[playerID].underAllin = true;
		float raiseBetVal = BBGC.allInInActionValue;
		Debug.Log("executeAllInCallForAIPlayer xxxxxxxxxxxxxxxxxxxxxx executeAllInCallForAIPlayer xxxxxxxxxxxxxxxxxxxxxx player : " + playerID + " raiseBetVal : " +raiseBetVal);
	...
	public IEnumerator executeAllIN(int playerID) {
		//Debug.Log("BBTurnPhaseAIController --->> executeAllIN @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@---- player : " + 
		//playerID + " pot : " + BBGC._BBGlobalDefinitions.moneyOnTable);

		float pot = BBGC._BBGlobalDefinitions.moneyOnTable;
		float playerCash = BBGC.playerDataList[playerID].currentPlayerTotalMoney;
        float extraPot = 0;

		if(playerCash < pot) {
		  extraPot = pot - extraPot;
		}

		    //Debug.Log("BBTurnPhaseAIController --->> executeAllIN @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@---- player : " + 
			//playerID + " pot : " + pot + " : " + " playerCash : " + playerCash + " extraPot : " + extraPot);

		BBGC.playerDataList[playerID].AllInImage.enabled = true;

		BBGC.allInInAction = true;
        BBGC.allInInActionExtraPotValue = extraPot;
        BBGC.allInInActionForPlayer = playerID;
        BBGC.allInInActionValue = playerCash;	
        BBGC.playerDataList[playerID].underAllin = true;

      yield break;
	}

	public void cleanAllInRequest() {

		for(int x = 0;x < BBGC.playerDataList.Count;x++) { 
		    BBGC.playerDataList[x].underAllin = false;
			BBGC.playerDataList[x].AllInImage.enabled = false;
		} 

		BBGC.allInInAction = false;
        BBGC.allInInActionExtraPotValue = 0;
        BBGC.allInInActionForPlayer = -1;
        BBGC.allInInActionValue = 0;	
  ...
  void Awake() {
	...
	IEnumerator showFinalPlayersResult() {
		...
		yield return StartCoroutine(GetComponent<BBShowDownResultController>().rotateCoveredCards());	  
		...
		for(int x = 0;x < playersCVal.Count;x++) {
			    ...
			    var sprite = Resources.Load<Sprite>("Avatar/playerAvatar_" + x.ToString());Image AvatarImage = GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("ImageAvatar").GetComponent<Image>();AvatarImage.overrideSprite = sprite;
			    GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("TextPlayerPosition").GetComponent<Text>().text = "("+(x+1).ToString()+")";

			      if(playersCVal[x].isRoundOut || BBGC.playerDataList[x].isOutOfGame) {
				   sprite = Resources.Load<Sprite>("star-fold");Image wonLoseImage = GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("ImageWonLose").GetComponent<Image>();wonLoseImage.overrideSprite = sprite;
			      } else {
				        if(playersCVal[x].won) {sprite = Resources.Load<Sprite>("star-won");Image wonLoseImage = GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("ImageWonLose").GetComponent<Image>();wonLoseImage.overrideSprite = sprite;}
				   else if(!playersCVal[x].won) {  sprite = Resources.Load<Sprite>("star-lose");Image wonLoseImage = GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("ImageWonLose").GetComponent<Image>();wonLoseImage.overrideSprite = sprite;}
			      }
			      ...
			      sprite = Resources.Load<Sprite>(humanIcon);Image wonLoseImage2 = GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("ImageHumanCpu").GetComponent<Image>();wonLoseImage2.overrideSprite = sprite;

			       GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("TextPlayerName").GetComponent<Text>().text = BBGC.playerDataList[x].T_playerName.text;

			      if(playersCVal[x].isRoundOut) {
				    outCounter++;
				      GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("TextResult").GetComponent<Text>().text = "?";
				      GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("TextBestFive").GetComponent<Text>().text = "?";
			      } else { 
				      if(BBGC.playerDataList[x].isOutOfGame) {
					     GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("TextResult").GetComponent<Text>().text = "?";
				         GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("TextBestFive").GetComponent<Text>().text = "?";
				      } else {
				         GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("TextResult").GetComponent<Text>().text = GetComponent<BBShowDownResultController>().getFormattedBestFive(playersCVal[x].bestFive);
				         GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("TextBestFive").GetComponent<Text>().text = playersCVal[x].CValue.ToString();
				      }
			      }
			       ...
			       if(playersCVal[x].won) {
			         if(playersCVal[x].playerPosID == BBGC._BBGlobalDefinitions.localPlayer) {
					   ...
					   localPlayerWonValue = playersCVal[x].splittedValue;
			            ...
			            if(BBGC.playerDataList[x].underAllin) {
						  goto GameEnds;
			            }
			         }
			         wonCounter++;
				      GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("TextMoneyResult").GetComponent<Text>().text = BBStaticData.getMoneyValue(playersCVal[x].splittedValue) + "/" + BBStaticData.getMoneyValue(BBGC.playerDataList[x].currentPlayerTotalMoney);
				      GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("TextMoneyResult").GetComponent<Text>().color = Color.green;
			       } else {
				      GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("TextMoneyResult").GetComponent<Text>().text = BBStaticData.getMoneyValue(BBGC.playerDataList[x].currentMoneyOnTable) + "/" + BBStaticData.getMoneyValue(BBGC.playerDataList[x].currentPlayerTotalMoney);
				      GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("TextMoneyResult").GetComponent<Text>().color = Color.red;

				      if(BBGC.playerDataList[x].currentPlayerTotalMoney < 50) {
				        BBGC._BBGlobalDefinitions.playersStateIsOutDuringOpenGame[x] = true;
				      }

			       }

			       if(playersCVal[x].kickerOnPlayerCard[0] != 0) {
				      GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("TextBestKicker").GetComponent<Text>().text = GetComponent<BBShowDownResultController>().getFormattedValue(playersCVal[x].kickerOnPlayerCard[0]);
			       }
		}
		      ...
		      int runOut = 0;
		         foreach(BBPlayerData bbpd in BBGC.playerDataList) {
		           if(bbpd.runOutOfMoney) runOut++;
		            ...
		            if(runOut > 8) {
				           BBGC.executeLocalPlayerWon(localPlayerWonValue,true);
				           yield break;
				    } else {
				        if(BBGC.playerDataList[BBGC._BBGlobalDefinitions.localPlayer].underAllin) {
					       BBGC.executeLocalPlayerWon(localPlayerWonValue,false);
				        } else {
			   ...
			   if(BBGC.playerDataList[BBGC._BBGlobalDefinitions.localPlayer].currentPlayerTotalMoney < 50) {
				   GetComponent<BBShowDownResultController>().finalResultPanel.SetActive(true);
				   BBGC.PanelLocalPlayerLose.SetActive(true);
				   BBGC.UIMoveingController.SetActive(true);
				   Transform bng =  BBGC.UIMoveingController.transform.Find("ButtonNewGame");
...
GameEnds:
		...
		BBGC.PanelLocalPlayerLose.SetActive(true);
		BBGC.UIMoveingController.SetActive(true);
		GameObject bng2 = GameObject.Find("ButtonNewGame");
		if(bng2 != null)
	        bng2.SetActive(false);
  ...
  void executePlayerWoneCoseAlone(int foldOutPlayers,int runOutPlayers) {
		Debug.Log("................................................... *** executeShowDowmAllin *** ...........executePlayerWoneCoseAlone..............................foldOut : " + foldOutPlayers +
		" runOut : " + runOutPlayers + " phase : " + BBGC._BBGlobalDefinitions.gamePhaseDetail);
		switch(BBGC._BBGlobalDefinitions.gamePhaseDetail) {
		case BBGlobalDefinitions.GamePhaseDetail.ShowDownAllInOnTurn:
		    cleanAllInRequest();
			StartCoroutine( BBGC.startExecuteTurnPhase() );
		break;
		case BBGlobalDefinitions.GamePhaseDetail.ShowDownAllInOnRiver:
		    cleanAllInRequest();
			StartCoroutine( BBGC.startExecuteRiverPhase() );
		break;
		}
  }
	...
	void executeRoundEnd(List<FinalResultStruct> winner) {
	   ...
	   foreach(FinalResultStruct r in winner) {
			Debug.Log("executeRoundEnd--------> cards : " + r.cardsResult + " best_1 : " + r.best_1 + " best_2 : " + r.best_2 + " best_3 : " + r.best_3 + " best_4 : " + r.best_4 + " PlayerID : " + r.playerID);
	   }
  ...
  public void executeShowDowmAllin() {

		int foldOutPlayers = (int)BBGC.getAllFoldAndRunOutPlayers().x;
        int runOutPlayers = (int)BBGC.getAllFoldAndRunOutPlayers().y;

        if(foldOutPlayers == BBGC.playerDataList.Count-1) {
			//executePlayerWoneCoseAlone(foldOutPlayers,runOutPlayers);
			//return;
        }

		if(runOutPlayers == BBGC.playerDataList.Count-1) {
			executePlayerWoneCoseAlone(foldOutPlayers,runOutPlayers);
			return;
        }
		    Debug.Log("................................................... *** executeShowDowmAllin *** ......................................... foldOutPlayers : " + foldOutPlayers +
		    ...
		    Debug.Log("[BBAllInController][executeShowDowmAllin] onlyOpenCardsValue =============>> : " + onlyOpenCardsValue + " : " + maxCardVal);
...
TestPairHighCard:
   ...
   public List<PlayersCValue> getCardsResultDuringGameHand() {
       List<PlayersCValue> temRet = new List<PlayersCValue>();
            ...
            int maxRes = 0;
                 if(BBGC.playerDataList[x].isOutOfGame) {
					 ...
					 temRet.Add(tmpV);
					    ...
					    Debug.Log("$$$$$$$$$$$$$$$$$$$$$$ res : " + res + " PId : " + x + " listCount : " + temRet.Count);
				    PlayersCValue tmpV = new PlayersCValue();
				     tmpV.playerPosID = x;
				     tmpV.CValue = res;
	...
	int[] getBestFive(BBPlayerCardsResultValueController.CardsValues cardval,int playerID,Vector2 card_1, Vector2 card_2) {
	    ...
	    Vector2[] allCards = null;
	         ...
	         if(simulateTwoPair[playerID]) { allCards = simulateCardValuerForPlayerID[playerID].simulateAllCardsValue;cardval = BBPlayerCardsResultValueController.CardsValues.TwoPair;} 
					...
					if(GetComponent<BBGameController>()._BBGlobalDefinitions.gamePhaseDetail == BBGlobalDefinitions.GamePhaseDetail.ShowDownAllInOnFlop ||
					   GetComponent<BBGameController>()._BBGlobalDefinitions.gamePhaseDetail == BBGlobalDefinitions.GamePhaseDetail.Flop) {
						Vector2[] tmpAllCards = new Vector2[5];
						tmpAllCards[0] = BBGC.flopCardList[0];
						tmpAllCards[1] = BBGC.flopCardList[1];
						tmpAllCards[2] = BBGC.flopCardList[2];
						tmpAllCards[3] = card_1;
						tmpAllCards[4] = card_2;
								allCards = tmpAllCards;
					}
			  else if(GetComponent<BBGameController>()._BBGlobalDefinitions.gamePhaseDetail == BBGlobalDefinitions.GamePhaseDetail.ShowDownAllInOnTurn ||
					  GetComponent<BBGameController>()._BBGlobalDefinitions.gamePhaseDetail == BBGlobalDefinitions.GamePhaseDetail.Turn) {
				        Vector2[] tmpAllCards = new Vector2[6];
						tmpAllCards[0] = BBGC.flopCardList[0];
						tmpAllCards[1] = BBGC.flopCardList[1];
						tmpAllCards[2] = BBGC.flopCardList[2];
						tmpAllCards[3] = BBGC.turnCard;
						tmpAllCards[4] = card_1;
						tmpAllCards[5] = card_2;
				           allCards = tmpAllCards;
					}
			  else if(GetComponent<BBGameController>()._BBGlobalDefinitions.gamePhaseDetail == BBGlobalDefinitions.GamePhaseDetail.ShowDownAllInOnRiver ||
					  GetComponent<BBGameController>()._BBGlobalDefinitions.gamePhaseDetail == BBGlobalDefinitions.GamePhaseDetail.River) {
				        Vector2[] tmpAllCards = new Vector2[7];
						tmpAllCards[0] = BBGC.flopCardList[0];
						tmpAllCards[1] = BBGC.flopCardList[1];
						tmpAllCards[2] = BBGC.flopCardList[2];
						tmpAllCards[3] = BBGC.turnCard;
						tmpAllCards[4] = BBGC.riverCard;
						tmpAllCards[5] = card_1;
						tmpAllCards[6] = card_2;
				          allCards = tmpAllCards;
					}
             else {
		...
		switch(cardval) {
		  case BBPlayerCardsResultValueController.CardsValues.HighCard:
			Debug.Log("<-------------->-->BBShowDownResultController--->>getBestFive-->getCompleteBestAfterHighCard playerID : " + playerID);
	...
	int[] getCompleteBestAfterHighCard(Vector2[] allCards, int playerID) {
		...
		if(valList[0] == 1) {
			tmpRes[0] = 1;
			tmpRes[1] = valList[valList.Count-1];
			tmpRes[2] = valList[valList.Count-2];
			tmpRes[3] = valList[valList.Count-3];
			tmpRes[4] = valList[valList.Count-4];
		} else {
			tmpRes[0] = valList[valList.Count-1];
			tmpRes[1] = valList[valList.Count-2];
			tmpRes[2] = valList[valList.Count-3];
			tmpRes[3] = valList[valList.Count-4];
			tmpRes[4] = valList[valList.Count-5];
		}
	...
	int[] getCompleteBestAfterPair(int pairVal,Vector2[] cardList, int playerID ) {
	   ...
	   List<int> valList = new List<int>();
		...
		//foreach(Vector2 i in cardList) if(logResult)  Debug.Log("---1---->>%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%getCompleteBestAfterPair : " + i.y + " pairVal : " + pairVal + " cardListLengh : " + cardList.Length);
		...
		//if(logResult) foreach(int i in valList) Debug.Log("----2--->>%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% valList lengh : " + valList.Count + " valListValue :" + i);

		   if(valList[0] == 1) {
			tmpRes[2] = 1;
			tmpRes[3] = valList[valList.Count-1];
			tmpRes[4] = valList[valList.Count-2];
		   } else {
			tmpRes[2] = valList[valList.Count-1];
			tmpRes[3] = valList[valList.Count-2];
			tmpRes[4] = valList[valList.Count-3];
...
//		foreach(int i in pairVal) Debug.Log("^^^^^^^^^^^^^^^^^^^pairVal^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ pairVal : " + i);

//		foreach(Vector2 i in cardList) if(logResult)  Debug.Log("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^------->>%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%getCompleteBestAfterTwoPair : " + i.y + " : " + pairVal[0] + " : " + pairVal[1]);

		for(int x = 0; x < cardList.Length;x++) {
			int result = -1;
//			Debug.Log("^^^^^^^^^^^^^^^^^^^pairVal^^^^^^^^1^^^^^^^^^^^^^^^^^^^^^^^^^^^ " + cardList[x]);
		   // if(valList.Count > 0) {
			  result = valList.Find(item => item == (int)cardList[x].y);
//			  Debug.Log("^^^^^^^^^^^^^^^^^^^pairVal^^^^^^^^2^^^^^^^^^^^^^^^^^^^^^^^^^^^ " + cardList[x].y + " result : " + result);
			//}
//			Debug.Log("^^^^^^^^^^^^^^^^^^^pairVal^^^^^^^^3^^^^^^^^^^^^^^^^^^^^^^^^^^^ " + cardList[x]);
...
//		foreach(int i in valList) if(logResult)  Debug.Log("--valList----->>%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%getCompleteBestAfterTwoPair valList.Sort() : " + i);


		if ((valList[0] == 1) && ( (pairVal[0] == 1) || (pairVal[1] == 1) ) ) {
			if(logResult)  Debug.Log("--valList----->> 1 <<-----");
			tmpRes[0] = pairVal[0];
		    tmpRes[1] = pairVal[0];
		    tmpRes[2] = pairVal[1];
		    tmpRes[3] = pairVal[1];
			...
			if(logResult)  Debug.Log("--valList----->> 2 <<-----");
			tmpRes[0] = pairVal[0];
		    tmpRes[1] = pairVal[0];
		    tmpRes[2] = pairVal[1];
		    tmpRes[3] = pairVal[1];
			...
			if(logResult)  Debug.Log("--valList----->> 3 <<-----");
			tmpRes[0] = pairVal[0];
		    tmpRes[1] = pairVal[0];
		    tmpRes[2] = pairVal[1];
		    tmpRes[3] = pairVal[1];
...
//		foreach(int i in tmpRes) if(logResult)  Debug.Log("RESULT--->>%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%getCompleteBestAfterTwoPair : " + i + " playerID : " + playerID);
		...
		for(int x = 0; x < cardList.Length;x++) {
		 ...
		 valList.Sort();
		   ...
		   if(valList[0] == 1) {
			...
			tmpRes[3] = 1;
			tmpRes[4] = valList[valList.Count-1];
		   } else {
			tmpRes[0] = ThreeOfAkindVal;
		    tmpRes[1] = ThreeOfAkindVal;
		    tmpRes[2] = ThreeOfAkindVal;
	...
	List<int[]> getFinalResultForFromFullHouse(List<PlayersCValue> winners) {
	 ...
	 List<Vector4> getFinalResultForTwoPair(Vector4[] tmpRes) {
		...
		for(int x = 0; x < tmpRes.Length;x++) {
			...
			for(int x = 0; x < tmpRes.Length;x++) {
			Debug.Log("payWinner==================>> pair 1 : " + tmpRes[x].x + " pair 2 : " + tmpRes[x].y + " kicker : " + tmpRes[x].z + " playerID : " + tmpRes[x].w);
				v4List.Add(tmpRes[x]);
			}
